...
import { View, Text, FlatList, StyleSheet, Pressable, Image, ActivityIndicator, Alert, Dimensions, TextInput, Button } from 'react-native';
...
export default function LostPetList({ userId }: { userId?: string }) {
    ...
    const [filterSize, setFilterSize] = useState<string | null>(null); // Added
    const [searchText, setSearchText] = useState<string>('');

    // UI State for Collapsible Filters
    const [activeFilter, setActiveFilter] = useState<'species' | 'sex' | 'region' | 'size' | null>(null); // Added size
    ...
    const fetchReports = async () => {
        try {
            setLoading(true);
            let url = `${API_URL}/api/reports?type=lost`;
            ...
            if (filterSize) url += `&size=${filterSize}`; // Added
            ...
            <View style={styles.itemContent}>
                <View style={styles.headerRow}>
                    <Text style={styles.itemTitle} numberOfLines={1}>
                        {item.name || item.species} {item.breed ? `• ${item.breed}` : ''}
                    </Text>
                ...
                <View style={styles.footerRow}>
                    {item.reward ? (
                        <Text style={styles.rewardText}>💰 賞金: {item.reward}</Text>
                    ) : (
                ...
                <FlatList
                    ...
                    ListEmptyComponent={
                        <View style={styles.emptyState}>
                            ...
                            <Button title="重新載入" onPress={fetchReports} color="#FF6B6B" />
                        </View>
                    }
                />
            )}
...
const styles = StyleSheet.create({
    ...
    filterButtonActive: { backgroundColor: '#FF6B6B', borderColor: '#FF6B6B' }, // Red for Lost
    ...
    optionChipActive: { backgroundColor: '#FF6B6B' },
    ...
    descText: { fontSize: 13, color: '#aaa', fontStyle: 'italic' },
    rewardText: { fontSize: 13, color: '#FF6B6B', fontWeight: 'bold' },
